    ...
    while (appRunning) {
       ...
       cin >> choice;
        ...
        switch (choice) {
            ...
            case 1: simState.currentAlgorithm = &dijkstra; break;
           
            default:cout << "Invalid choice. Defaulting to A*.\n"; simState.currentAlgorithm = &dijkstra; break;
        }
        ...
        while (Graphics::ProcessMessages()) {
            ...
            if (Graphics::IsKeyDown('S')) {
                Node* n = grid.GetNode(gx, gy);
                if (n) grid.SetStartNode(n);
            }
            if (Graphics::IsKeyDown('E')) {
                Node* n = grid.GetNode(gx, gy);
                if (n) grid.SetEndNode(n);
            }

           

            if (Graphics::IsKeyDown(' ')) { // Space to start
            ...
            if (Graphics::IsKeyDown('C')) { // Clear Path only
                grid.ResetPath();
                simState.running = false;
                simState.finished = false;
            }
